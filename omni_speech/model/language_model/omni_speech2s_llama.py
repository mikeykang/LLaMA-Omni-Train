...
class OmniSpeech2SLlamaForCausalLM(OmniSpeechLlamaForCausalLM, GenerationWithCTC):
    ...
    def initialize_speech_generator(self, model_args):
        self.config.speech_generator_type = getattr(
            model_args, "speech_generator_type", "ctc"
        )
        self.config.ctc_decoder_config = getattr(
            model_args, "ctc_decoder_config", "(4,4096,32,11008)"
        )
        ...
        self.tune_speech_generator_only = getattr(
            model_args, "tune_speech_generator_only", False
        )
    ...
    def forward(
        ...
        if self.training:
            if self.tune_speech_generator_only:
                ...
                loss = self.speech_generator(
                    llama_output["hidden_states"][-1], labels, tgt_units
                )
            else:
                llama_output = super(OmniSpeechLlamaForCausalLM, self).forward(
                    input_ids=input_ids,
                ...
                ctc_loss = self.speech_generator(
                    llama_output["hidden_states"][-1], labels, tgt_units
                )
